...
int main()
{
    ...
    std::random_device r; //urzadzenie do generowania entropii (zalezne od systemu operacyjnego)
    std::default_random_engine defEngine(r()); //tworzymy sobie silnik losowosci, ktory bedzie zwracac nam losowe liczby
    std::uniform_int_distribution<int> intDistro(-10, 10); // ubiermy ten nasz silnik w klase zapewniajaca nam odpowiedni rozklad liczb
    for (int i = 1; i <= 10; i++)
    {
